import {
  ...
  function calculateHomeAffordability(input: FinancialInput) {
    ...
    const excessEmergencyFund = Math.max(
      input.emergencyFund - protectedEmergencyFund,
      0
    );
  ...
  function scoreHomeAffordability(input: FinancialInput) {
    ...
    if (data.downPaymentGap > 0) {
      ...
      addDriver(
        ...
        "Down payment gap",
        -25,
        "The user does not have enough available cash for the modeled down payment."
      );
    } else {
      addDriver(
        drivers,
        ...
        0,
        "The user appears to have enough available cash for the down payment."
      );
    }
  
    if (data.housingCostRatio > 0.3) {
      score -= 15;
      addDriver(
        drivers,
    ...
    if (data.housingCostRatio > 0.4) {
      ...
      addDriver(
        ...
        -15,
        "Housing costs are highly stretched relative to monthly income."
      );
    }
    ...
    return {
      ...
      metrics: [
        ...
        },
        {
          ...
          detail: base.data.downPaymentGap > 0 ? "Additional cash needed" : "No gap",
        },
        {
          label: "Monthly Housing Cost",
          value: formatCurrency(base.data.estimatedMonthlyHousingCost),
    ...
    return {
      ...
      metrics: [
        ...
        },
        {
          ...
          detail: base.retirementGap >= 0 ? "Projected surplus" : "Projected shortfall",
        },
        {
          label: "Coverage Ratio",
          value: `${base.coverageRatio.toFixed(2)}x`,
  ...
  function scoreInvestAssets(input: FinancialInput) {
    ...
    if (emergencyFundGap > 0) {
      ...
      addDriver(
        ...
        "Emergency fund gap",
        -25,
        "The emergency fund is below the 3-month expense target."
      );
    } else {
      addDriver(
        drivers,
        ...
        0,
        "The emergency fund meets or exceeds the 3-month expense target."
      );
    }
  ...
  function analyzeInvestAssets(input: FinancialInput): GoalAnalysis {
    ...
    const scenarios = [
        ...
        [
          {
            ...
            value: formatCurrency(Math.max(conservative.emergencyFundGap, 0)),
          },
          {
      ...
      buildScenario("Base", base.score, "Current user assumptions.", [
        {
          label: "Emergency Fund Gap",
          value: formatCurrency(Math.max(base.emergencyFundGap, 0)),
        },
        {
          label: "Monthly Surplus",
      ...
      buildScenario(
        ...
        [
          {
            label: "Emergency Fund Gap",
            value: formatCurrency(Math.max(optimistic.emergencyFundGap, 0)),
          },
          {
            label: "Monthly Surplus",
    ...
    return {
      ...
      goalScore: base.score,
      scoreAdjustment: getScoreAdjustment(base.score),
      marginOfSafety: `${liquidityCoverage.toFixed(1)} months of expenses`,
      summary:
        status === "strong"
      ...
      metrics: [
        ...
        {
          label: "Margin of Safety",
          value: `${liquidityCoverage.toFixed(1)} months`,
          detail: "Emergency fund coverage in months of expenses",
        },
      ],
      decisionRules: [
  ...
  function scoreScholarship(input: FinancialInput) {
    ...
    const annualIncomeBenefit = input.income * (input.expectedIncomeIncrease / 100);
